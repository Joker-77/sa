...
export const CreateProduct = ({ direction, product, isCreate, backToPrevious }) => 
{
    ...
    function handleChange(e) {
      ...
      const handleSubmit = (values, props) => {
        ...
        if(isCreate)
        {
            ...
            ProductsService.updateProduct(product.id, data).then(resp => { 
                toast.success(resp.message);
            }).catch(error => toast.error("An error"))
        }
    };
    useEffect(() => {
        console.clear()
        StoresService.getAllStores().then(res => {
            console.log(res)
            setStores(res.data);
                    if(!isCreate)
                        setStore(res.data.find((c) => c.id === initialValue?.store_id));
            }).catch(error => { })
        CategoriesService.getAllCategories().then(resp => {
            setCategories(resp.data);
            if(!isCreate)
                setCategory(resp.data.find((c) => c.id === initialValue?.category_id));
        CouponsService.getAllCoupons().then(resps => {
            setCoupons(resps.data);
                    if(!isCreate)
                        setCoupon(resps.data.find((c) => c.id === initialValue?.coupon_id));
            }).catch(error => { })
        }).catch(error => { })
    }, []);
    ...
    return <React.Fragment>
                ...
                >
                     {(props) => {
                        const { en_name, ar_name, en_description, ar_description, coupon_id, 
                                selling_price, unit_price, entity, brand, category_id, store_id } = props.values;
                return (
                  <Form>
