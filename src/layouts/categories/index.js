...
function Categories() {
  ...
  const onSearchChange = useAsyncDebounce((value) => {
    setSearch(value);
    if (!!value) {
      let _searched = categories.filter(e.name.includes(value) || e.name_a.includes(value));
      setFiltered(_searched);
    } else setFiltered(categories);
  }, 100);
  //
  ...
  return (
    <DashboardLayout>
      {!show && (
        <>
          {" "}
          <DashboardNavbar name={direction == "rtl" ? "الأصناف" : "Categories"} />
      ...
      {show && (
        <>
          <DashboardNavbar
            name={direction == "rtl" ? "إضافة أو تعديل صنف" : "Create or Edit Category"}
          />
          <CreateCategory
            direction={direction}
            isCreate={createNew}
