...
function Users() {
  ...
  const getUsers = (type) => {
    ...
    setUsers([]);
    if (type == "users")
      UsersService.getAllUsers().then((resp) => {
        console.log(resp);
        setFiltered(resp);
        setUsers(resp);
      });
    if (type == "client")
      UsersService.getAllClients().then((resp) => {
        console.log(resp);
        setFiltered(resp.data);
        setUsers(resp.data);
      });
    if (type == "marketers")
      UsersService.getAllMarketers().then((resp) => {
        console.log(resp);
        setFiltered(resp.data);
        setUsers(resp.data);
      });
  ...
  const types = ["users", "marketers", "client"];
  ...
  return (
    <DashboardLayout>
      {!show && (
        <>
          {" "}
          <DashboardNavbar name={direction == "rtl" ? "المستخدمون" : "Users"} />
      ...
      {show && (
        <>
          <DashboardNavbar
            name={direction == "rtl" ? "إضافة أو تعديل مستخدم" : "Create or Edit User"}
          />
          <CreateUser
            direction={direction}
            isCreate={createNew}
